...
namespace _1brc.gen_data
{
    ...
    internal static class Program
    {
        private static void Main( string[] args )
        {
            try
            {
                //CreateMeasurements2.Run( @"E:\1brc-main\1brc_1B.txt", size: 1_000_000_000 );
                CreateMeasurements2.Run( @"E:\1brc-main\1brc_100M.txt", size: 100_000_000 );
            }
            catch ( Exception ex )
            {
    ...
    /// 
    /// </summary>
    internal static class CreateMeasurements2
    {
        /// <summary>
        /// 
        /// </summary>
        private struct WeatherStation
        {
             ...
             */
            public void measurement( StringBuilder buf )
            {
                // fake -10.9 to +10.9 variance without double operations and rounding
                // gives us -10 to +10
                int m = meanTemperature + (r.Next( 21 ) - 10);
                // gives us a decimal digit 0 to 9 as char
                char d = NUMBERS[ r.Next( 10 ) ];

                // just append, only one number has to be converted and we can do
                // better... if we watn
                buf.Append( firstPart, 0, firstPart.Length ).Append( m ).Append( '.' ).Append( d ).Append( '\n' );
            }
        }

        public static void Run( string fileName, int size = 1_000_000_000 )
        {
            var stations = new[] {
            ...
            using var file = File.OpenWrite( fileName ); file.SetLength( 0 );
            ...
            {
                produce( sw, stations, strideSize );

                Console.WriteLine( $"{i + 1}. of {outer}" );
                sw.Flush();
            }
            // there might be a rest
            produce( sw, stations, remainder );
        }

        private static void produce( StreamWriter sw, IList<WeatherStation> stations, int count )
        {
            ...
            {
                {
                    ...
                    int s1 = r1.Next( stationCount );
                    int s2 = r2.Next( stationCount );
                    int s3 = r3.Next( stationCount );
                    int s4 = r4.Next( stationCount );
